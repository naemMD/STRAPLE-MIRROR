  ...
  const fetchCoachData = async () => {
    try {
      ...
      if (session?.id) {
        ...
        try {
          const clientsRes = await api.get(`/coaches/${session.id}/clients`);
  ...
  return (
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false} keyboardDismissMode="on-drag">
      ...
      <View style={styles.card}>
        <Text style={user?.description ? styles.cardDesc : styles.cardDescEmpty}>
          {user?.description || "No description yet. Tap Edit Profile to add one."}
        </Text>
      </View>
      ...
      <View style={styles.metricsRow}>
        ...
        <View style={styles.metricCard}>
          ...
          <Text style={styles.metricValue} numberOfLines={1}>{user?.city || '--'}</Text>
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
            <ScrollView showsVerticalScrollIndicator={true} keyboardShouldPersistTaps="handled" nestedScrollEnabled>
              ...
              <TextInput
                ...
                onChangeText={(t) => setEditDescription(t.slice(0, 150))}
                placeholder="Short bio for your clients..."
                placeholderTextColor="#666"
                multiline
              />
              <Text style={{ color: '#888', fontSize: 11, textAlign: 'right', marginTop: 4 }}>{editDescription.length}/150</Text>
...
const styles = StyleSheet.create({
  ...
  coachEmail: { color: '#888', fontSize: 13, marginTop: 4 },
  coachCity: { color: '#3498DB', fontSize: 12, marginTop: 4 },
  ...
  cardDesc: { color: '#ccc', fontSize: 14, lineHeight: 22, fontStyle: 'italic' },
  cardDescEmpty: { color: '#666', fontSize: 13, fontStyle: 'italic' },
  ...
  modalContent: { backgroundColor: '#1A1F2B', borderRadius: 20, padding: 20, maxHeight: '85%', borderWidth: 1, borderColor: '#2A4562' },
